class VoteCommsController < ApplicationController
	...
	def create
		...
		if VoteComm.where(user: current_user, comment: Comment.find(params[:vote_comm][:comment_id])).count > 0
			VoteComm.where(user: current_user, comment: Comment.find(params[:vote_comm][:comment_id])).destroy_all
		end

		@votecomm.user = current_user
		@votecomm.comment = Comment.find(params[:vote_comm][:comment_id])
